...
tap <-
  tapirs %>%
  bind_rows(
    bind_cols(name = c('Atlantic forest', 'Cerrado', 'Pantanal', 'Overall'),
  ...
  mutate(name = factor(name, levels = c(unique(tapirs$name), 'Atlantic forest',
         ...
         region.lab = factor(region.lab,
                             levels = c('Atlantic forest', 'Cerrado', 'Pantanal',
                                        'Overall')),
...
plot_grid(plot.akde('Atlantic Forest'), plot.akde('Cerrado'), plot.akde('Pantanal'),
          ncol = 2) # used ncol = 1 for figure in the main text
```
...
ggplot(tap) +
  ...
  scale_y_discrete(limits = rev,
                   ...
                   breaks = c('Atlantic forest', 'PA_33_GABRIELA', 'CE_15_KURUKA',
                              'AF_14_JAMESBOND')) +
...
plot_grid(plot.akde('Atlantic Forest'), plot.akde('Cerrado'),
          plot.akde('Pantanal'), ncol = 2) # used ncol = 1 for figure in the main text
```
...
ggplot() +
  ...
  scale_color_manual('Region', values = pal[1:3], 
                     labels = c('Atlantic forest', 'Cerrado', 'Pantanal')) +
  labs(x = 'Proportion of exposed soil in the habitat',
       y = expression('Home Range Area'~(km^2))) +
  theme(legend.position = 'top',
        panel.grid = element_blank())
