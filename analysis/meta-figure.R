...
tapirs <- readRDS('models/tapirs-final.rds') %>% arrange(name)
...
tap <-
  ...
  bind_rows(
    bind_cols(
      name = c('Atlantic forest', 'Cerrado', 'Pantanal', 'Overall'),
      region.lab = c('Atlantic forest', 'Cerrado', 'Pantanal', 'Overall'),
      bind_rows(
  ...
  mutate(
    ...
    name = factor(name,
                  levels = c(unique(tapirs$name), 'Atlantic forest',
                             'Cerrado', 'Pantanal', 'Overall')),
    region.lab = factor(region.lab,
                        levels = c('Atlantic forest', 'Cerrado', 'Pantanal', 'Overall')),
    average = if_else(name %in% c('Atlantic forest', 'Cerrado', 'Pantanal', 'Overall'),
...
p.areas <-
  ...
  scale_y_discrete(limits = rev,
                   ...
                   breaks = c('Atlantic forest', 'CE_15_KURUKA', 'PA_33_GABRIELA',
                              'AF_14_JAMESBOND')) +
...
p.tau.pos <-
  ...
  scale_y_discrete(limits = rev,
                   labels = c('Means', 'Pantanal', 'Cerrado', 'Atlantic forest'),
...
p.tau.vel <-
  ggplot(tap) +
  geom_segment(aes(x = tau.velocity.low,
                   xend = tau.velocity.high, y = name, yend = name,
                   color = region.lab), lwd = 2) +
  ...
  scale_shape_manual(element_blank(), values = c(19, 17)) +
  scale_color_manual('Region', values = c(pal[1:3], 'black'),
                     labels = c('Atlantic forest', 'Cerrado', 'Pantanal', 'Overall')) +
  scale_y_discrete(limits = rev,
                   labels = c('Means', 'Cerrado', 'Pantanal', 'Atlantic forest'),
                   breaks = c('Atlantic forest', 'CE_15_KURUKA',
                              'PA_33_GABRIELA', 'AF_14_JAMESBOND')) +
